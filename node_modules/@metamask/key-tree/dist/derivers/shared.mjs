 ...
 */
export async function deriveChildKey({ path, node, curve, network }, handleError, cryptographicFunctions) {
    validateNode(node);
 ...
 */
export async function deriveSecretExtension({ privateKey, childIndex, isHardened, curve, }) {
    if (isHardened) {
        // Hardened child
        return concatBytes([
            new Uint8Array([0]),
 ...
 */
export function derivePublicExtension({ parentPublicKey, childIndex, }) {
 ...
 * @returns The derived {@link SLIP10Node}.
 */
export async function derivePublicChildKey({ entropy, publicKey, depth, masterFingerprint, parentFingerprint, childIndex, curve, network, }, cryptographicFunctions) {
 ...
 */
export function privateAdd(privateKeyBytes, tweakBytes, curve) {
    assert(isValidBytesKey(tweakBytes, 32), 'Invalid tweak: Tweak must be a non-zero 32-byte Uint8Array.');
...
function validatePath(path, node, curve) {
    ...
    assert(!isHardened || node.privateKey, 'Invalid parameters: Cannot derive hardened child keys without a private key.');
    assert(isHardened || curve.deriveUnhardenedKeys, `Invalid path: Cannot derive unhardened child keys with ${curve.name}.`);
}
/**
 ...
 */
export function getValidatedPath(path, node, curve) {
    validatePath(path, node, curve);
    const indexPart = path.split(`'`)[0];
    assert(indexPart);
    const childIndex = parseInt(indexPart, 10);
    if (!UNPREFIXED_PATH_REGEX.test(indexPart) ||
        !Number.isInteger(childIndex) ||
        childIndex < 0 ||
        childIndex >= BIP_32_HARDENED_OFFSET) {
        throw new Error(`Invalid path: The index must be a non-negative decimal integer less than ${BIP_32_HARDENED_OFFSET}.`);
    }
